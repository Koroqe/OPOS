...
- **Self-improvement log schema lacked a terminal status for no-delta runs.** `status: open | applied | rejected` — a run with `proposed_delta: none` could only be marked `open` and then never closed (the other two assert a delta existed); downstream measured two entries sitting `open` for 41 days. Added `n/a` (REQUIRED when `proposed_delta` is `none`) to `CLAUDE.md.jinja`, `README.md.jinja`, `shared/templates/PROCESS.md.tmpl` (both occurrences), `shared/templates/scheduled-run.md.tmpl` and `departments/rnd/.claude/skills/deploy/PROCESS.md` (the one shipped PROCESS.md that restates the schema inline). Note: root `CLAUDE.md` is `_skip_if_exists`, so existing consumers must copy the schema line by hand; new scaffolds get it.
