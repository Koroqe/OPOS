...
The ops-manager does NOT execute the process themselves — that's the owner agent's job once the process is live. **As of v0.4.0**, the ops-manager also owns `design-agent` (the parallel skill for creating new agent files); when a designed process needs an agent role that doesn't exist, the ops-manager invokes `design-agent` inline instead of escalating to `coo`. **As of v0.5.3**, ops-manager owns the FULL design family: `design-process` (new skills), `design-agent` (new roles), and `design-department` (new dept folders + lead-agent delegation recommendation). The trio closes the org-chart-expansion loop end-to-end — ops-manager can generate any framework primitive from natural-language input. **As of v0.6.0**, ops-manager ALSO owns the full scheduling family: `schedule-process` / `unschedule-process` / `list-scheduled-processes`. These three wrap Claude Code's built-in `CronCreate` / `CronDelete` / `CronList` tools and turn any process with the 4 scheduling frontmatter fields (`schedule`, `runtime`, `non_interactive`, `authority`) into a cron-fired routine. ops-manager now owns BOTH meta-design (3 design-* skills) AND meta-scheduling (3 scheduling skills) — 6 owned skills total. **As of v0.8.0**, ops-manager owns the COMPLETE org-chart-shape design family: `design-process` (skills v0.1.0), `design-agent` (roles v0.4.0), `design-department` (top-level depts v0.5.3), `design-subdept` (sub-depts v0.8.0). The quartet covers every primitive a founder might need to grow the org chart from natural-language input — skills, agents, top-level departments, and sub-departments. ops-manager now owns 8 skills total (4 meta-design + adopt-proposal + 3 meta-scheduling). `coo` escalation remains the fallback when the user rejects a design.

**Designing for the autonomous operating model (v0.19.0)** — per [`company/policies/autonomy-and-decision-rights.md`](../../../company/policies/autonomy-and-decision-rights.md):

- **Every designed process says who does what and how it is checked.** Fill the optional `PROCESS.md` fields `executor:` (the agent that runs it), `checker:` (who verifies the result and with how many independent methods — usually `result-checker` via `check-result`), `decision_classes:` (the R0–R3 classes of the actions it takes), and, when it takes any R3 action, `autonomy_ladder:` (`off` / `shadow` / `on` + cap; the rung is the holder's decision, never the agent's). Its body carries a `## Verification` section.
- **Every designed agent has a zone** — its lease key, its labels, its processes — and a named checker. A new role is justified by a separate zone, not by load: more load means more running instances of an existing role, each under its own lease.
- **Consult with purpose, not sparingly.** A consultation costs cents; a design that missed the department that runs it costs a redesign. Consult every department the process touches.
...
- For sub-department gaps (a sub-organization within an existing top-level dept, with its own charter/data/backlog scope) — invoke `design-subdept` (v0.8.0+) to create the nested folder + charter. Same emit-not-invoke pattern as design-department for the sub-lead recommendation. Closes RISKS Risk 8 fully-fully (third + final tier).
...
- `design-department` — `.claude/skills/design-department/` (NEW in v0.5.3) — closes the new-top-level-dept-charter gap. 12-step procedure (ceo + coo always consulted; optional one specific dept-lead/sub-lead). Top-level depts only. Emits a `/design-agent` recommendation when the user wants the dept's lead designed in the same session.
- `design-subdept` — `.claude/skills/design-subdept/` (NEW in v0.8.0) — closes the sub-dept gap. 12-step procedure mirroring design-department; takes a `parent_dept` input + sub-dept description; consults parent-lead + coo + (optional) related peer; renders from new `SUBDEPT.md.tmpl` (forked from DEPARTMENT.md.tmpl with depth-3 paths); writes charter at `departments/<parent>/<sub>/CLAUDE.md[.jinja]`; cascade-inherits from parent + root. Sub-lead recommended via emit-not-invoke `/design-agent`. Sub-sub-depts (depth > 2) intentionally NOT supported.
