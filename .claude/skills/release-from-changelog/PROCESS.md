...
success_criteria: [changelog_entry_found, extract_non_empty, no_existing_release_for_version, prerelease_scaffold_check_passed, release_created, release_verified, history_entry_written]
slo: "1-2 minutes (interactive — gh API + git operations + copier scaffold smoke)"
version: 0.2.0
---
...
Mirrors the 8-step procedure in SKILL.md (8 steps as of v0.2.0 of this skill — pre-release scaffold check added):
...
5. Pre-release scaffold check (Copier scaffold against HEAD must exit 0).
6. Cut release via `gh release create`; auto-derive title from first non-reserved `### ` subheading.
7. Verify creation.
...
- `prerelease_scaffold_check_passed` — step 5 Copier scaffold exited 0.
- `release_created` — step 6 `gh release create` returned 0.
- `release_verified` — step 7 `gh release view --json tagName` returned the version.
