...
- **Choose the write path** by `gh api repos/<owner>/<repo> --jq .permissions.push`:
  ...
  - `false` (third-party consumer): `gh repo fork <owner>/<repo> --clone=false --default-branch-only` — the fork lands in the **user account, never a company org** (an org fork would publish the company name, the exact thing anonymization forbids).
    **Base the branch on upstream, never on the fork (v0.18.0).** A fork's own `main` is only as fresh as the last time someone clicked "Sync fork"; a branch cut from it produces a PR whose diff was drafted against upstream HEAD (step 4a) but whose base is old code — conflicts at best, a PR that silently reverts newer upstream changes at worst. So:
    1. best-effort, bring the fork current with the contributor's own credentials: `gh repo sync <user>/<repo> --source <owner>/<repo> --branch <default>` (their token carries the `workflow` scope the Actions token lacks);
    2. **whatever step 1 did,** shallow-clone the **upstream** repo, create the branch there, `git remote add fork https://github.com/<user>/<repo>.git`, and push the branch to `fork`;
    3. if that push is rejected for missing `workflow` scope (the branch carries upstream commits that touched workflow files), run `gh auth refresh -s workflow` once and push again.
