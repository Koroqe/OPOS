...
4b. **Runtime dispatch.** `runtime: claude-schedule` → continue with steps 5–9 below (CronCreate path). `runtime: cloud` → take the CLOUD branch (immediately below). `runtime: gha` → take the GHA branch after it.
   ...
   **CLOUD branch (`runtime: cloud`) — the recommended durable path:**
   ...
   - **Register** via `RemoteTrigger` `{action: "create", body: {...}}`: `name: opos-<process-name>`, `cron_expression` = the declared cron **shifted to UTC** with the minute re-rolled for fleet jitter (cloud crons are UTC; minimum interval 1 hour), `enabled: true`, and `job_config.ccr` = `{environment_id: <the operator's environment>, session_context: {model, sources: [{git_repository: {url: <repo https URL>}}], allowed_tools: <derived from the commands: manifest>}, events: [<one user event carrying the composed prompt, with a fresh lowercase v4 uuid>]}`.
   - **Human gate (the authorization moment):** show the composed prompt, cron (local + UTC), model, repo, and allowed tools; register only on explicit confirmation. Never register from a scheduled run (never-automate invariant 3).
   ...
   - Then skip to step 9a (cache row `{"process_name", "routine_id": "cloud:<trigger_id>", "cron", "registered_at"}`) and step 10 (history entry). Print the routine's URL `https://claude.ai/code/routines/<trigger_id>`.
