...
6. **Draft the design.** Fill in the SKILL.md and PROCESS.md templates with:
   ...
   - **Operating-model fields (v0.19.0, optional but expected)** — per `company/policies/autonomy-and-decision-rights.md`: `executor:` (the role whose workers run it), `checker:` (who verifies, with how many independent methods — usually `result-checker` via `check-result`; at least 2 for R2), `decision_classes:` (the R0–R3 classes of its actions), and `autonomy_ladder:` whenever it takes an R3 action (default `off`; only the holder of the right moves it). Fill the PROCESS.md `## Verification` section to match. A process whose result nobody but its executor checks is not ready to adopt.
