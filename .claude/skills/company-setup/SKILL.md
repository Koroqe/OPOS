...
8b. **Activate the self-improvement loop (v0.10 — the step that makes the company autonomous).** Ask: "Your OS can update itself daily and review its own improvement signals weekly, unattended. Activate now? (yes/no — recommended yes)." On yes:
   - If the repo has a GitHub remote: run `/schedule-process auto-sync` and `/schedule-process review-history` (both declare `runtime: cloud` by default — durable, billed to the founder's Claude subscription, no API key; the one prerequisite is connecting their GitHub account to claude.ai, which the registration surfaces if missing). Consumers who prefer CI-side execution and already have Anthropic API billing can switch either PROCESS.md to `runtime: gha` before registering.
