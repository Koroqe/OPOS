...
9b. **Lease gate (v0.17.0).** `node .claude/skills/lease/lease.mjs check --key "issue:<repo>#<number>"`. Handle the exit code exactly as in the lease skill's "Calling the lease from another skill" table: `0` proceed · `9` the company has not opted in to leases — print `lease: not configured, gate skipped` and proceed exactly as before · anything else STOP. Closing someone else's task from another machine is exactly the collision leases exist to stop. On exit `4` or `5`, `acquire` the key: `0` (nobody holds it) → proceed, `2` → stop and report the holder.
