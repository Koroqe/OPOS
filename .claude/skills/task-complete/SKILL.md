...
9b. **Lease gate (v0.17.2).** `node .claude/skills/lease/lease.mjs acquire --key "issue:<repo>#<number>" --intent "completing"` — a no-op exit `0` if this session already holds the task, a take if it is free, and exit `2` in every enforcement mode if another session holds it: then STOP and report the holder. Closing someone else's task from another machine is exactly the collision leases exist to stop. `9` = not opted in: skip the gate and proceed as before. `3` = stale clone: `git pull --ff-only`, retry.
...
12b. **Release the lease.** `node .claude/skills/lease/lease.mjs release --key "issue:<repo>#<number>" --reason "completed"`. This also removes the index label. Exit `9` is fine; any other non-zero is reported but does not reopen the issue — the lease would lapse on its TTL anyway.
...
14. **Remove the completed issue from the local cache.** `node shared/scripts/task-state.mjs remove-active --issue "$ISSUE_NUM"`. An already-absent entry exits 0, so a concurrent `task-complete` from another session is harmless. Other active tasks are untouched.
