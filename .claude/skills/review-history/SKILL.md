...
3. **Select work:** all entries with `status: open` and `proposed_delta` ≠ `none`, minus those with a live `upstream_pr` (skipped in step 2).
4. **Classify each delta:** by the validated `delta_target:` hint; else infer the target path from the `proposed_delta` text; else run the two-part runtime classification test (identical to `propose-to-core` step 2 — pinned-`_commit` `copier.yml` fetch for `_skip_if_exists`, HEAD existence probe; fetch once per run, memoize). Unclassifiable → triage note, leave `open`, move on.
