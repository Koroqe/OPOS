...
1b. **Q0 — instrument or executor? (v0.12 pre-check).** Before the executor tree: is the gap actually a missing TOOL/ACCESS (an account, an API, credentials, browser-drivable capability) rather than a missing role? Signals: the gap names a system ("DNS", "Cloudflare", "email sending", "our CRM") or an access verb ("we can't log into / edit / publish X"). If instrument → file/increment the `kind: resource-gap` backlog item, then invoke `/acquire-resource` for it inline (v0.13) and stop with `outcome: routed_to_acquire_resource`. Mixed gaps (need a role AND its tools) → continue the tree AND route the instrument half to `/acquire-resource`.
