...
9b. **Take the lease on the new issue (v0.17.0).** This is what makes the task visible as *in progress* to every other session, on every machine — `.current-task` never could:
   ```bash
   node .claude/skills/lease/lease.mjs acquire --key "issue:<repo>#$ISSUE_NUM" --intent "<title>"
   ```
   Handle the exit code exactly as in the lease skill's "Calling the lease from another skill" table: `0` proceed · `9` the company has not opted in to leases — print `lease: not configured, gate skipped` and proceed exactly as before · anything else STOP. The issue already exists at this point, so on a non-zero exit say so plainly ("issue #N created, but NOT leased: <reason>"), finish the remaining steps, and record `outcome: partial`. Exit `3` (stale clone) means run `git pull --ff-only` and re-acquire — never work on a task from a clone that is behind.

10. **Record the issue in the local cache.** `node shared/scripts/task-state.mjs add-active --issue "$ISSUE_NUM"`. Since v0.17.0 `.current-task` is a **local convenience cache** — it remembers which issues this clone is working on so later steps need no `--issue` — and is no longer the source of truth for occupancy: the lease from step 9b is. The script replaces the old `echo >>` append, so the concurrent-append race of RISKS 30 is gone with it.
